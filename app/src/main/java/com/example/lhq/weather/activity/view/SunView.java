package com.example.lhq.weather.activity.view;
...
//日出日落时间画笔
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (!isDraw) {
            ...
            // 渐变路径
            mPathShader = new LinearGradient(mWidth / 2, svPadding, mWidth / 2, endY, mainColor, Color.WHITE, Shader.TileMode.CLAMP);
...
//        canvas.drawRect((float) rX, 0, mWidth, mHeight, mShadePaint);
        ...
        if (mSunrise.length != 0 || mSunset.length != 0) {
            ...
            canvas.drawText("日出 " + (mSunrise[0] < 10 ? "0" + mSunrise[0] : mSunrise[0]) + ":" + (mSunrise[1] < 10 ? "0" + mSunrise[1] : mSunrise[1]), startX + textOffset, startY-2, mTextPaint);
            mTextPaint.setTextAlign(Paint.Align.RIGHT);
            canvas.drawText("日落 " + (mSunset[0] < 10 ? "0" + mSunset[0] : mSunset[0]) + ":" + (mSunset[1] < 10 ? "0" + mSunset[1] : mSunset[1]), endX - textOffset, endY-2, mTextPaint);
        }
        // 画端点
    ...
    /**
     * 设置当前进度，并更新太阳中心点的位置     * @param t 范围：[0~1]
     */
    ...
    public void setCurrentTime(int hour, int minute) {
        if (mSunrise.length != 0 || mSunset.length != 0) {
            float p0 = mSunrise[0] * 60 + mSunrise[1];
            // 起始分钟数
            float p1 = hour * 60 + minute - p0;
            // 当前时间总分钟数
            float p2 = mSunset[0] * 60 + mSunset[1] - p0;
            // 日落到日出总分钟数
