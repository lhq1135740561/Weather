package com.example.lhq.weather.activity.network.event;

import com.example.lhq.weather.activity.network.event.base.BaseEvent;

/**
 * BaseActionEvent 即用于向 View 层传递 Action 的 Model，
 * 在 ViewModel 通过向 View 层传递不同的消息类型，从而触发相对应的操作。
 * 因此，BaseViewModel 需要向子类提供默认的实现。
 */

public class BaseActionEvent extends BaseEvent {

    public static final int SHOW_LOADING_DIALOG = 1;

    public static final int DISMISS_LOADING_DIALOG = 2;

    public static final int SHOW_TOAST = 3;

    public static final int FINISH = 4;

    public static final int FINISH_WITH_RESULT_OK = 5;

    private String message;

    public BaseActionEvent(int action) {
        super(action);
    }

    public String getMessage() {
        return message;
    }

    public void setMessage(String message) {
        this.message = message;
    }
}
